            ...
            while True:
                ...
                if response_dict:
                    try:
                        ...
                        log.error('Scan step failed ({:d}). Trying again.'.format(step))
                    except:
                        traceback.print_exc()
                        log.error('Scan step failed ({:d}). Trying again.'.format(step))
                else:
                    log.info('Map Download failed ({:d}). Trying again.'.format(step))
...
def search(args, queue):
    ...
    for step_location in generate_location_steps(position, num_steps):
        task = {'step':step, 'step_location':step_location}
        queue.put(task)
        step += 1
